...
async function workflowStats(id: string): Promise<void> {
...
function remapNodeForSave(n: AnyRecord): AnyRecord {
  ...
  if (n.entryType) node.entryType = n.entryType;
  if (n.fields != null) node.fields = n.fields;
  ...
  for (const key of ALL_CONFIG_KEYS) {
    if (n[key] != null) node[key] = n[key];
  }

  return node;
}

function remapConnectionForSave(c: AnyRecord): AnyRecord {
  return {
...
async function workflowAddNode(args: string[]): Promise<void> {
  ...
  if (!nodeType) error("--type is required");
  ...
  const existing = (await api("GET", `/api/workflow/${workflowId}`)) as AnyRecord;
  const nodes = ((existing.nodes as AnyRecord[]) || []).slice();
  const connections = (existing.connections as AnyRecord[]) || [];

  // Generate node ID
  const nodeId = `${nodeType}_${Date.now()}`;

  const newNode: AnyRecord = { id: nodeId, type: nodeType, x, y, label };
  if (nodeType === "entry") {
  ...
  const result = (await api("POST", `/api/workflow/${workflowId}`, body)) as AnyRecord;
...
async function workflowUpdateNode(args: string[]): Promise<void> {
  const workflowId = getArg(args, "--workflow");
  ...
  if (!workflowId) error("--workflow is required");
  if (!nodeId) error("--node is required");

  const existing = (await api("GET", `/api/workflow/${workflowId}`)) as AnyRecord;
  const nodes = (existing.nodes as AnyRecord[]) || [];
  const connections = (existing.connections as AnyRecord[]) || [];

  const found = nodes.some((n) => n.id === nodeId);
  if (!found) error(`Node '${nodeId}' not found in workflow ${workflowId}`);

  const saveNodes = nodes.map((n) => {
    const remapped = remapNodeForSave(n);
    if (n.id === nodeId) {
      if (newLabel) remapped.label = newLabel;
      if (newXStr) remapped.x = parseFloat(newXStr);
      if (newYStr) remapped.y = parseFloat(newYStr);
      if (newEntryType) remapped.entryType = newEntryType;
      if (newFields) {
        try { remapped.fields = JSON.parse(newFields); } catch { error("Invalid JSON for --fields"); }
      }
      if (newConfig) {
        try {
        ...
        } catch { error("Invalid JSON for --config"); }
      }
    }
    return remapped;
  });
...
async function workflowListNodes(id: string): Promise<void> {
  const data = (await api("GET", `/api/workflow/${id}`)) as AnyRecord;
  const nodes = (data.nodes as AnyRecord[]) || [];
...
async function workflowRemoveNode(args: string[]): Promise<void> {
  const workflowId = getArg(args, "--workflow");
  const nodeId = getArg(args, "--node");
  ...
  const saveConnections = connections
    .filter((c) => {
      const from = (c.fromNodeId || c.from) as string;
      const to = (c.toNodeId || c.to) as string;
...
async function workflowConnect(args: string[]): Promise<void> {
  const workflowId = getArg(args, "--workflow");
  ...
  const existing = (await api("GET", `/api/workflow/${workflowId}`)) as AnyRecord;
  const nodes = (existing.nodes as AnyRecord[]) || [];
  const connections = ((existing.connections as AnyRecord[]) || []).map(remapConnectionForSave);

  const newConn: AnyRecord = { from, to };
  if (sourceHandle) newConn.sourceHandle = sourceHandle;
...
function extractVarRefs(s: string): Array<[string, string]> {
  ...
  while (true) {
    ...
    if (root) refs.push([varName, root]);
    start = close + 2;
  }
  return refs;
}

async function workflowValidate(id: string): Promise<void> {
  const existing = (await api("GET", `/api/workflow/${id}`)) as AnyRecord;
  const nodes = (existing.nodes as AnyRecord[]) || [];
  const connections = (existing.connections as AnyRecord[]) || [];

  interface Issue { level: string; node: string | null; message: string }
  const issues: Issue[] = [];

  if (nodes.length === 0) {
    issues.push({ level: "error", node: null, message: "Workflow has no nodes" });
    if (outputJson) { printJson({ valid: false, issueCount: 1, issues }); } else {
  ...
  const labelVarMap = new Map<string, string>();
  ...
  // Adjacency
  const incoming = new Map<string, AnyRecord[]>();
  const outgoing = new Map<string, AnyRecord[]>();
  for (const n of nodes) {
    const nid = String(n.id || "");
    incoming.set(nid, []);
    outgoing.set(nid, []);
  ...
  const ancestorsOf = (nodeId: string): Set<string> => {
    ...
    for (const c of incoming.get(nodeId) || []) {
      const from = String(c.fromNodeId || c.from || "");
      if (from && !visited.has(from)) { visited.add(from); q.push(from); }
    }
    while (q.length > 0) {
      const cur = q.shift()!;
  ...
  for (const n of nodes) {
    ...
    if (nodeType === "ai") {
      const aiCfg = (config.aiConfig || {}) as AnyRecord;
      if (!aiCfg.userPrompt) {
    ...
    if (nodeType === "email") {
      const emailCfg = (config.emailConfig || {}) as AnyRecord;
      const to = emailCfg.to as string[] | undefined;
    ...
    if (nodeType === "condition") {
      ...
      if (out.length === 0) {
        ...
        for (const c of out) {
          const handle = c.sourceHandle as string || "";
          if (!handle) {
    ...
    if (varRefs.length > 0) {
      ...
      const ancestorVarNames = new Set<string>();
      for (const aid of ancestors) {
        const vn = labelVarMap.get(aid);
        if (vn) ancestorVarNames.add(vn);
      }
      ...
      for (const [fullRef, root] of varRefs) {
        if (root === "entry") {
          ...
          for (const aid of ancestors) {
            ...
            if (!ancestorNode || ancestorNode.type !== "entry") continue;
            let aCfg: AnyRecord = {};
            if (typeof ancestorNode.config === "string") {
              try { aCfg = JSON.parse(ancestorNode.config); } catch { /* */ }
            }
            const fields = (aCfg.fields as Array<AnyRecord>) || [];
...
async function workflowGenerate(description: string, args: string[]): Promise<void> {
  const model = getArg(args, "--model") || "google/gemini-2.0-flash-001";
  const workflowId = getArg(args, "--workflow");

  const body: AnyRecord = { prompt: description, model };
  if (workflowId) body.workflowId = parseInt(workflowId, 10);

  const result = (await api("POST", "/api/chat/workflow", body)) as AnyRecord;

  if (outputJson) {
    printJson(result);
  } else {
    if (result.response) print(String(result.response));
    const proposed = result.proposedWorkflow as AnyRecord | undefined;
    if (proposed) {
...
async function templateList(args: string[]): Promise<void> {
  ...
  const data = (await api("GET", `/api/templates?${params.join("&")}`)) as AnyRecord;

  if (outputJson) {
    printJson(data);
  } else {
    const templates = (data.templates as AnyRecord[]) || [];
...
async function templateGet(id: string): Promise<void> {
  const data = await api("GET", `/api/templates/${id}`);
  printJson(data);
}

async function templateUse(id: string, args: string[]): Promise<void> {
  const name = getArg(args, "--name");
  const body: AnyRecord = {};
  if (name) body.workflowName = name;

  const data = (await api("POST", `/api/templates/${id}/use`, body)) as AnyRecord;

  if (outputJson) {
    printJson(data);
  } else {
    const wf = data.workflow as AnyRecord | undefined;
    const wfId = wf?.id;
...
async function templateCreate(args: string[]): Promise<void> {
  ...
  const data = (await api("POST", "/api/templates", body)) as AnyRecord;

  if (outputJson) {
    printJson(data);
  } else {
    const tmpl = data.template as AnyRecord | undefined;
    printSuccess(`Template '${name}' created (id: ${tmpl?.id})`);
  }
}

async function templateDelete(id: string): Promise<void> {
...
async function integrationList(): Promise<void> {
  if (outputJson) {
...
async function integrationInfo(id: string): Promise<void> {
  const meta = INTEGRATIONS.find((i) => i.id === id);
  if (!meta) error(`Unknown integration: ${id}`);
...
async function guide(args: string[]): Promise<void> {
  const sub = args[0];
...
async function workflowVisualize(id: string): Promise<void> {
  const data = (await api("GET", `/api/workflow/${id}`)) as {
    name?: string;
  ...
  if (outputJson) {
    ...
    const graphEdges = connections.map((c) => {
      ...
      const handle = c.sourceHandle as string | undefined;
  ...
  // Build lookup maps
  const nodeInfo = new Map<string, { label: string; type: string }>();
  for (const n of nodes) {
    const nid = n.id as string;
    nodeInfo.set(nid, {
      label: (n.label as string) || "(unlabeled)",
      type: (n.type as string) || "?",
    });
  }

  // Adjacency
  const childrenMap = new Map<string, Array<{ to: string; handle: string }>>();
  ...
  const formatNode = (nid: string): string => {
  ...
  for (let i = 0; i < layers.length; i++) {
    ...
    if (i < layers.length - 1) {
      ...
      for (const arrow of arrows) print(arrow);
      print("");
    }
  }
  ...
  switch (cmd) {
    ...
    case "wf":
      switch (sub) {
        ...
        case "stats":
          if (!rest[0]) error("Usage: zeitflow workflow stats <id>");
          return workflowStats(rest[0]);
        case "validate":
          if (!rest[0]) error("Usage: zeitflow workflow validate <id>");
          return workflowValidate(rest[0]);
        case "generate":
          if (!rest[0]) error("Usage: zeitflow workflow generate \"<description>\"");
        ...
        case "list-nodes":
        case "nodes":
          if (!rest[0]) error("Usage: zeitflow workflow list-nodes <id>");
        ...
        case "open":
          if (!rest[0]) error("Usage: zeitflow workflow open <id>");
          return workflowOpen(rest[0]);
        case "visualize":
        case "viz":
          if (!rest[0]) error("Usage: zeitflow workflow visualize <id>");
    ...
    case "tpl":
      switch (sub) {
        ...
        case "get":
          if (!rest[0]) error("Usage: zeitflow template get <id>");
          return templateGet(rest[0]);
        case "use":
          if (!rest[0]) error("Usage: zeitflow template use <id>");
          return templateUse(rest[0], rest.slice(1));
        case "create":
          return templateCreate(rest);
        case "delete":
        case "rm":
          if (!rest[0]) error("Usage: zeitflow template delete <id>");
    ...
    case "int":
      switch (sub) {
        ...
        case "info":
          if (!rest[0]) error("Usage: zeitflow integration info <id>");
