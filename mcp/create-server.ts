 ...
 *
 * Creates a fully-configured McpServer with all ZeitFlow tools registered.
 * Used by both the stdio entry-point (mcp/server.ts) and the HTTP endpoint
 * (pages/api/mcp.ts) so that the tool definitions live in one place.
 */
...
/** Minimal Drizzle database interface – works with any node-postgres instance */
 ...
 *
 * @param db      A Drizzle node-postgres database instance.
 * @param user    The authenticated user (resolved from API token).
 * @param opts    Optional overrides (host URL for execute_workflow).
 */
export function createMcpServer(
  db: DrizzleDB,
  user: McpUser,
  opts?: { host?: string; apiToken?: string }
): McpServer {
  ...
  const apiToken = opts?.apiToken || process.env.ZEITFLOW_API_TOKEN || "";
  ...
  server.tool(
    ...
    async ({ status, limit }) => {
      ...
      const baseCondition = eq(workflowsTable.userId, user.id);
  ...
  server.tool(
    ...
    async ({ workflowId }) => {
      const [workflow] = await db
        ...
        .where(
          and(
            ...
            eq(workflowsTable.userId, user.id)
          )
        )
        .limit(1);
      ...
      const parsedNodes = nodes.map((node: { config: string | null } & Record<string, unknown>) => ({
        ...node,
  ...
  server.tool(
    ...
    "Update a node's label, position, or configuration",
    {
      ...
      config: z
        ...
        .optional()
        .describe(
          "New configuration object (replaces existing config)"
        ),
      entryType: z
        .enum(["api", "form", "webhook"])
        .optional()
  ...
  server.tool(
    ...
    "Execute a workflow. The workflow must have at least one node. Returns the execution ID and output data.",
    {
      ...
      inputData: z
        ...
        .optional()
        .describe(
          "Input data for entry nodes (key-value pairs matching field keys)"
        ),
      entryNodeId: z
        .string()
        .optional()
    ...
    async ({ executionId }) => {
      const [execution] = await db
        ...
        .where(
          and(
            ...
            eq(workflowExecutionsTable.userId, user.id)
          )
        )
        .limit(1);
      ...
      const parsed = {
        ...
        inputData: execution.inputData
          ? JSON.parse(execution.inputData)
          : null,
  ...
  server.tool(
    ...
    async ({ workflowId, limit, status }) => {
      ...
      const whereClause = status
        ? and(
            baseCondition,
            eq(workflowExecutionsTable.status, status)
          )
        : baseCondition;
        ...
        .array(
          z.object({
            ...
            positionX: z
              ...
              .int()
              .optional()
              .describe("X position (default: auto)"),
            positionY: z
              .number()
              .int()
              .optional()
              .describe("Y position (default: auto)"),
            entryType: z
              .enum(["api", "form", "webhook"])
              .optional()
              .describe("Entry type (only for entry nodes)"),
              ...
              content: [
                {
                  type: "text" as const,
                  text:
                    `Invalid connection: "${conn.from}" -> "${conn.to}". ` +
